...
async fn railcom_uart_runtime_dispatch_task(
    ...
    loop {
        ...
        match output {
            RailcomUartRuntimeOutput::Adapter(RailcomUartAdapterOutput::WindowProcessed(
                result,
            )) => {
                if !matches!(result.outcome, RailcomRxOutcome::Empty) {
                    debug!("railcom uart: {:?}", result);
                }

                if let Some(sighting) = crate::railcom::loco_tracker::record_loco_identification(
                ...
                let direct_sighting = crate::railcom::loco_tracker::record_loco_identification(
                    result.window.packet_sequence,
                    result.items.as_slice(),
                ) && pom_result_sender.try_send(pom_result).is_err()
                {
                    warn!("railcom uart: pom result channel full");
                }
            }
            RailcomUartRuntimeOutput::Adapter(RailcomUartAdapterOutput::WindowCorrupted(
                window,
            )) => {
                debug!(
            ...
            RailcomUartRuntimeOutput::Adapter(other) => {
                warn!("railcom uart adapter: {:?}", other);
            }
            RailcomUartRuntimeOutput::ReaderError(err) => {
                warn!("railcom uart reader: {:?}", err);
            }
            RailcomUartRuntimeOutput::ReaderNoise(noise) => {
                if noise.after_first_byte {
                    info!(
                        "railcom uart noise: {:?} packet={} channel={:?} after_first_byte=1",
                        noise.kind, noise.packet_sequence, noise.channel
                    );
                }
            }
        }
...
#[embassy_executor::task]
async fn pom_cutout_monitor_task(
    pom_tx_started_sender: Sender<'static, CriticalSectionRawMutex, u32, 1>,
    railcom_window_sender: Sender<'static, CriticalSectionRawMutex, RailcomWindowControl, 8>,
) -> ! {
    ...
    loop {
        ...
        let mut events = heapless::Vec::<CutoutRuntimeEvent, 8>::new();
        ...
        for event in events {
            match event {
                CutoutRuntimeEvent::Opened {
                    ...
                    railcom_window_sender
                        .send(RailcomWindowControl::Open {
                            packet_sequence,
                            channel: crate::railcom::pipeline::RailcomChannel::Channel2,
                        })
                        .await;
                    if pom_requested {
                        pom_tx_started_sender.send(packet_sequence).await;
                    }
                }
                CutoutRuntimeEvent::Closed => {
                    railcom_window_sender
                        .send(RailcomWindowControl::Close)
                        .await;
                }
            }
        }
