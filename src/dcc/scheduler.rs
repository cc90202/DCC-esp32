...
/// Minimum number of transmitted packets between two RailCom cutouts.
...
impl RailcomCutoutBudget {
    ...
    fn allow_cutout_for(&mut self, class: PacketClass) -> bool {
        let eligible = matches!(class, PacketClass::Refresh | PacketClass::Idle);
        let allowed = eligible && self.packets_since_cutout >= RAILCOM_MIN_PACKET_GAP;
...
#[cfg(test)]
fn reset_railcom_scheduler_stats() {
...
impl SlotManager {
    ...
    fn build_next_packet_classified_with_function_budget(
        &mut self,
        max_slot_visits_without_function: u8,
...
pub async fn packet_scheduler_task(
    ...
    loop {
        ...
        let to_send = DccFrame::new(packet, railcom_budget.allow_cutout_for(class));
...
mod tests {
    ...
    fn test_scheduler_classifies_dirty_speed_as_command() {
        ...
        let (packet, class) = mgr
            .build_next_packet_classified_with_function_budget(
                allowed_slot_visits_without_function(1),
            )
